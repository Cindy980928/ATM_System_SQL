 ...
 */
	public User singIn(){
		...
		System.out.println("开始用户注册，请输入你的名字：");
			//创建姓名
			user_singin.setName(Input.getString());
			//更新数据库
			Userdao.updataUser(user_singin);
			//创建密码
			user_singin.setPassword(Input.getPassword());
			...
			if("y".equals(key)){
				...
				//创建预存款
				user_singin.setBalance(prestore);
				//更新数据库
				Userdao.updataUser(user_singin);
